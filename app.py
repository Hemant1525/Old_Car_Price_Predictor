...
# ---------------------------------------------------------
# Train model directly from car_price.csv (same logic as Model.ipynb)
# Cached so training runs only ONCE when the app starts, not on every click
# ---------------------------------------------------------
@st.cache_resource
def load_model():
    raw_df = pd.read_csv("car_price.csv")

    df = raw_df.drop(['car_prices_in_rupee'], axis=1)
    ...
    bundle = {
        "model": rf,
        "scaler": ss,
        "car_name_le": car_name_le,
        "fuel_type_le": fuel_type_le,
        "transmission_le": transmission_le,
        "feature_columns": feature_columns,
        "unique_car_names": sorted(raw_df['car_name'].unique().tolist()),
        "unique_fuel_types": sorted(raw_df['fuel_type'].unique().tolist()),
        "unique_transmissions": sorted(raw_df['Transmission'].unique().tolist()),
        "manufacture_min": int(raw_df['Manufacture'].min()),
        "manufacture_max": int(raw_df['Manufacture'].max()),
    }
    return bundle

with st.spinner("Model train ho raha hai... (sirf pehli baar, thoda time lagega)"):
    bundle = load_model()
